...
class _ContractListScreenState extends State<ContractListScreen>
  ...
  Future<void> _load() async {
    ...
    try {
      final items = await _contractApi.getMobileContracts();
  ...
  List<MobileContract> get _loanItems =>
      _allItems.where((c) => c.isForLoan).toList();

  List<MobileContract> get _savingItems =>
      _allItems.where((c) => c.isForSaving).toList();

  Future<void> _viewContract(MobileContract item) async {
    try {
      final detail = await _contractApi.getMobileContractDetail(item.id);

      if (detail.fileUrl != null && detail.fileUrl!.isNotEmpty) {
        final uri = Uri.parse(detail.fileUrl!);
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          title: Text(
            detail.contractNumber?.isNotEmpty == true
          ...
          content: SizedBox(
            ...
            child: SingleChildScrollView(
              child: Text(
                detail.renderedBody?.trim().isNotEmpty == true
                    ? detail.renderedBody!
                    : 'Không có nội dung hợp đồng để hiển thị.',
      ...
      if (!mounted) return;
      _snack('Không thể xem hợp đồng: $e', isError: true);
    }
  }

  Future<void> _confirmSign(MobileContract item) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
          'Bạn xác nhận đã đọc kỹ hợp đồng và đồng ý với tất cả điều khoản. Sau khi ký, hệ thống sẽ hoàn tất nghiệp vụ tương ứng.',
          style: TextStyle(fontSize: 13, height: 1.5),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(false),
            child: const Text('Hủy', style: TextStyle(color: _C.secondary)),
          ),
          FilledButton(
    ...
    if (ok == true) await _doSign(item);
  }

  Future<void> _doSign(MobileContract item) async {
    try {
      final updated = await _contractApi.signMobileContract(item.id);
      if (!mounted) return;
      setState(() {
  ...
  Widget _buildList(List<MobileContract> items) {
    if (items.isEmpty) {
      return const Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.description_outlined, size: 48, color: _C.secondary),
          SizedBox(height: 10),
          Text('Chưa có hợp đồng nào', style: TextStyle(color: _C.secondary, fontSize: 13)),
        ]),
      );
    }
  ...
  Widget _buildCard(MobileContract c) {
    final isLoan = c.isForLoan;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          child: Row(children: [
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text(_ownerLabel(c), style: const TextStyle(fontSize: 11, color: _C.secondary)),
              ]),
            ),
            _statusBadge(c),
          ]),
        ...
        Padding(
          ...
          child: Column(children: [
            _infoRow(Icons.tag_rounded, 'Mã tham chiếu', '#${c.ownerId}'),
            _infoRow(Icons.insert_drive_file_outlined, 'Loại', isLoan ? 'Hợp đồng vay vốn' : 'Chứng nhận tiết kiệm'),
            _infoRow(Icons.schedule_rounded, 'Ngày tạo', _fmtDate(c.createdAt)),
            if (c.signedAt != null) _infoRow(Icons.draw_outlined, 'Ngày ký', _fmtDate(c.signedAt)),
          ]),
        ),
        Padding(
  ...
  Widget _statusBadge(MobileContract c) {
    final (label, color, bg) = switch (c.status.toLowerCase()) {
  ...
  Widget _infoRow(IconData icon, String label, String value) => Padding(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          SizedBox(width: 110, child: Text(label, style: const TextStyle(fontSize: 12, color: _C.secondary))),
          Expanded(child: Text(value, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: _C.primary))),
        ]),
      );

  String _ownerLabel(MobileContract c) {
    return switch (c.ownerType.toLowerCase()) {
      'loan_application' => 'Hồ sơ vay #${c.ownerId}',
